...
class PocketOptionBot:
    ...
    async def fetch_candles(self, symbol: str, timeframe: int, candles_to_check: int) -> pd.DataFrame:
        counter = 0
        data = None
        while data is None:
            data = await self.api.get_candles(symbol, timeframe, None, timeframe * candles_to_check)
            counter += 1
            if counter == 5:
                raise FetchingCandlesMultipleAttemptsException(
                    "Error fetching candles")
        return data

    async def execute_order(self, amount: int, symbol: str, action: str, expiration_seconds: int) -> None:
        result = False
        counter = 0
        while not result:
